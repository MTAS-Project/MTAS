...
public class MyCustomPhoneListener extends PhoneStateListener
{
  	...
  	public int asuToLevel(int asu)
	{
		...
		else if(asu>2 && asu<=5)	{	level = 1;	}
		else if(asu>5 && asu<=8)	{	level = 2;	}
		else if(asu>8 && asu<=12)	{ 	level = 3;	}
	...
	{
		String quality ="";
		if(level==0)		{ 	quality = "No Service";	}
		else if(level==1)	{	quality = "Weak";	}
		else if(level==2)	{	quality = "Fair";	}
		else if(level==3)	{ 	quality = "Good";	}
		else if(level==4)	{	quality = "Excellent";	}
